...
int32_t main() {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        dp[i][1] = (float) (dp[i][0].first + a[i].second) / max(dp[i][0].second, a[i].first) >= (float) (dp[i][1].first + a[i].second) / max(dp[i][1].second, a[i].first) ? make_pair(dp[i][0].first + a[i].second, max(dp[i][0].second, a[i].first)) : make_pair(dp[i][1].first + a[i].second, max(dp[i][1].second, a[i].first));
